...
import { getSlip } from '@/api/client';
...
export async function generateMetadata({
  ...
  const slip = await getSlip(id, owned);
  if (!slip) return { title: 'Slip not found' };
  return { title: `${slip.package.name} slip` };
}

export default async function SlipPage({ params }: PageProps<'/pro/slips/[id]'>) {
  const { id } = await params;
  const [owned, session] = await Promise.all([getOwnedSlipIds(), getSession()]);
  const slip = await getSlip(id, owned);
  if (!slip) notFound();
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      ...
      {slip.unlocked ? (
        ...
        <section className="mt-6 rounded-xl border border-line bg-surface p-6">
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-line-soft pt-5">
            <div>
            ...
            {session ? (
              <form action={purchaseSlipAction}>
          ...
          <p className="mt-4 text-xs leading-relaxed text-fg-dim">
            Placeholder checkout — this unlocks the slip immediately and charges
            nothing. Real MarzPay mobile-money collection lands with the Go
            backend.
          </p>
        </section>
      )}
