...
export type Region = 'europe' | 'east-africa' | 'africa' | 'americas' | 'asia';
...
export interface FreeTip {
  ...
  odds: number;
  /** Present once the pick has been graded. */
  result?: PredictionResult;
}
...
/**
 * A past shortlist with its results attached — "yesterday went 4 from 5".
 *
 * `settled` is the number of tips that have been graded, which lags
 * `totalTips` while a matchday is still in progress. `hitRate` is over
 * `settled`, not over `totalTips`, or an unfinished day would read as a bad
 * one.
 */
export interface FreeTipsHistoryDay {
  date: string;
  totalTips: number;
  settled: number;
  correct: number;
  /** 0..1, over the settled tips. */
  hitRate: number;
  groups: FreeTipGroup[];
}
...
/**
 * One attempt to buy one slip.
 *
 * `refunded` is a distinct status from `failed`: the money arrived and was
 * sent back, and `paidAt` stays populated because when it arrived is part of
 * the record. Only `paid` grants access.
 */
...
/** What `POST /slips/{id}/purchase` returns while the collection is in flight. */
export interface PurchaseResult {
  purchaseId: string;
  status: string;
  /**
   * The short code that appears in the payer's SMS and on the MarzPay
   * statement. Shown to the user so support can match a payment from it
